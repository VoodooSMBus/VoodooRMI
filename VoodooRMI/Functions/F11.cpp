...
bool F11::getReport()
{
    ...
    abs_size = nbr_fingers & RMI_F11_ABS_BYTES;
...
int F11::f11_2d_construct_data()
{
    ...
    nbr_fingers = (query->nr_fingers == 5 ? 10 :
                           query->nr_fingers + 1);
    
    pkt_size = DIV_ROUND_UP(nbr_fingers, 4);
    
    if (query->has_abs) {
        pkt_size += (nbr_fingers * 5);
        attn_size = pkt_size;
    }
    
    if (query->has_rel)
        pkt_size += (nbr_fingers * 2);
